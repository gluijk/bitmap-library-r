...
ClearBitmap = function(img, inc=FALSE, val=0) {
  # Limpia bitmap
...
CopyRect = function(img, x0, y0, x1, y1, xdst, ydst, inc=TRUE, val=1) {
...
ShowBitmap = function(img, trunc=TRUE, gamma=1, chan=2, interpolate=FALSE) {
...
for (n in 1:5) img=DrawCircle(img,
    250+n*10, 200+n*7, n^0.9*30, val=0.1, fill=TRUE)
img=DrawRect(img, 75, 30, 125, 360, val=0.5, fill=TRUE)
...
for (t in 0:(NPERIODO*CICLOS-1)) {
  ...
  for (n in 1:NTERMINOS) {
    ...
    frm=DrawCircle(frm, CENTRO+x[n], ALTO/2+y[n], abs(radio[n]), val=0.18, fill=TRUE)  # Círculo
    frm=DrawLine(frm, CENTRO+x[n], ALTO/2+y[n], CENTRO+x[n+1], ALTO/2+y[n+1], inc=FALSE)  # Radio
  }
  
  frm=DrawLine(frm, CENTRO+x[n+1], ALTO/2+y[n+1], ANCHO, ALTO/2+y[n+1], val=0.2)  # Línea hasta gráfica
  frm=DrawPoint(frm, ANCHO+1, ALTO/2+y[n+1], inc=FALSE)  # Punto de la gráfica
  ...
  frm=CopyRect(frm, ANCHO+1, 1, nrow(frm)-1, ncol(frm), ANCHO+2, 1, inc=FALSE)
  frm=DrawRect(frm, 1, 1, ANCHO+1, ALTO, inc=FALSE, val=0, fill=TRUE)
}
